...
class Settings(BaseSettings):
    ...
    # Nuovo campo per l'URI di SQLAlchemy: se impostato (es. per test), viene usato
    ...
    PAYMENT_SUCCESS_URL: str = Field(
        default=os.getenv("PAYMENT_SUCCESS_URL", "http://localhost:3000/payment/success"),
        description="URL di reindirizzamento dopo un pagamento riuscito"
    )
    PAYMENT_CANCEL_URL: str = Field(
        default=os.getenv("PAYMENT_CANCEL_URL", "http://localhost:3000/payment/cancel"),
        description="URL di reindirizzamento dopo un pagamento annullato"
    )
    ...
    RATE_LIMIT_ENABLED: bool = Field(
        ...
        description="Abilita/disabilita il rate limiting sulle API"
    )
    DEFAULT_RATE_LIMIT: str = Field(
        default=os.getenv("DEFAULT_RATE_LIMIT", "100/minute"),
        description='Limite di default (es: "100/minute")'
    )
    ...
    def build_sqlalchemy_uri(cls, values: dict) -> dict:
        if not values.get("SQLALCHEMY_DATABASE_URI"):
            # Se non è definita, costruisce l'URI MySQL a partire dalle altre variabili
